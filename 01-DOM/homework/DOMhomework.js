...
function buildToDo(todo, index) {
  // Tu código acá:
  //Creo un 'div' y se lo asigno a una variable toDoShell
  let toDoShell = document.createElement('div');
  //Le asigno a toDoShell la clase toDoShell
  toDoShell.classList.add('toDoShell');
  //Creo un elemento span y se lo asigno a una variable toDoText
  let toDoText = document.createElement('span');
  //Utilizo el objeto toDo seteo el toDoText y le asigno el valor de description
  toDoText.innerHTML = toDo.description;
  //Asigno como id del toDoText el valor index
  toDoText.id = index;
  //Agrego un click, tomando el elemento y obtengo su valor de id, ejecutando la funcion "completeToDo"
  toDoText.addEventListener("click", completeToDo);
  //Retorno dependiendo si es true o false
  if(toDo.complete){
    //Si es asi agrego una clase:
    toDoText.setAttribute("click","completeText");
  } 
  //Agrego toDoText como hijo de toDoShell
...
function buildToDos(toDos) {
  // Tu código acá:
  //Genero un array que dentro tiene "div's"
  let newToDos = toDos.map((toDo, index) => {
    return buildToDo(toDo, index); // Llamamos a la función previamente creada
  });
...
function displayToDos() {
  // Tu código acá:
  //Seleccionar el elemento cuyo id es 'toDoContainer' y almacenarlo en una variable denominada 'toDoContainer'
  let toDoContainer = document.getElementById("toDoContainer")
  //Seteo el innner como string vacio
  toDoContainer.innerHTML = "";
  //Creo un array que dentro tiene tareas(div's)
  let resultado = buildToDos(toDoItems)
  //Le indico a map que quiero recorrer el array y por cada elemento lo inserto dentro del contenedor "toDoContainer"
  resultado.map(elemento => toDoContainer.appendChild(elemento))
}
...
function completeToDo(event) {
  ...
  // Tu código acá:
  //Cuando haga click en una "tarea" va a pasar de complete: false --> complete: true y se le asigna una clase(complete)
  toDoItems[index].completeToDo();
  //Agrego un event(click) 
  displayToDos();
}
